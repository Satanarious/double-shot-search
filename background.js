  ...
  for (var i = 0; i < details.requestHeaders.length; ++i) {
    if (
      ...
      details.requestHeaders[i].value =
        details.requestHeaders[i].value + " Edg/111.0.1661.44";
    }
...
chrome.omnibox.onInputEntered.addListener(function (query) {
  if (query.startsWith("chat ")) {
    var searchUrl = `https://www.bing.com/search?q=${encodeURIComponent(
      query.substring(5)
    )}&showconv=1&sendquery=1&FORM=hpcodx`;
    var bardUrl = "https://bard.google.com/";
    chrome.tabs.create({ url: bardUrl }, function (tab) {
      var bardTabId = tab.id;
      chrome.tabs.executeScript(bardTabId, {
        code:
...
chrome.webRequest.onBeforeRequest.addListener(
  function (details) {
    if (
      details.url.indexOf(
        "https://www.doubleshotsearch.download/chrome-extension/search/?q="
      ) !== -1 ||
      details.url.indexOf(
        "chrome-extension://" + chrome.runtime.id + "/?q="
      ) !== -1
    ) {
